...
import { EXCHANGE } from "src/lib/constants";
...
/**
 * published events on queue
 */
const publish_event = async (routingKey: string, payload: any) => {
  const conn = await amqplib.connect(env.QUEUE_URL);
...
export const sendEmails = async (
  ...
  await publish_event("email.send", {
    to: email,
    subject,
    body,
  });
